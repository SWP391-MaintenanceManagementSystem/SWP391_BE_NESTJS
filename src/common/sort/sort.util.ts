...
import { ACCOUNT_SORTABLE_FIELDS, Order, ROLE_SORTABLE_FIELDS, VEHICLE_SORTABLE_FIELDS } from './sort.config';
...
export function buildAccountOrderBy(
    ...
    const field =
        ACCOUNT_SORTABLE_FIELDS[sortBy] ||
        ROLE_SORTABLE_FIELDS[role]?.[sortBy];
    ...
    return JSON.parse(
        JSON.stringify(field).replace(/"asc"/g, `"${order}"`)
    );
}
